...
\begin{equation}
\begin{aligned}
    \Delta F_{e} &= \frac{i_{0}^2}{2} \cdot \frac{d^2L(x_{0})}{dx^2} \cdot \Delta x + i_{0} \cdot \frac{dL(x_{0})}{dx} \cdot \Delta i \\
...
Para analizar el sistema hidráulico, se procede a definir dos fuerzas, la entrante (i.e., la salida del sistema del solenoide) y otra de salida (i.e., la fuerza de presión que se genera en el pistón 2). Para esto, se define la siguiente ecuación.
\begin{equation}
    F_{out} = ( \frac{F_e}{A_1}-\rho g x ) A_2
\end{equation}

Donde $F_e$ corresponde a la fuerza descrita en la sección anterior.

Visto desde el punto de vista del usuario, de una prensa se espera escoger un valor de presión, por lo que la ecuación que define el valor buscado es la siguiente.
\begin{equation}
    p_{out} = \frac{F_{out}}{A_2} = \frac{F_e}{A_1}-\rho g x=\frac{i^2}{2} \frac{dL}{dx} \frac{1}{A_1}-\rho g x
\end{equation}
