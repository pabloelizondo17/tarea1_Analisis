...
El circuito de control tiene como objetivo regular la tensión promedio 
aplicada al solenoide mediante una señal de modulación por ancho de 
pulso (PWM). En este esquema, un interruptor 
controlado (transistor MOSFET en saturación) conmuta la 
tensión de alimentación $V_{cc}$ a una frecuencia fija $f_s$, de manera 
que el valor promedio de la tensión en la carga queda determinado 
únicamente por el ciclo de trabajo $D \in [0,1]$.
...
Dado que la bobina del solenoide actúa intrínsecamente como un filtro 
paso bajo de primer orden, con una frecuencia de corte 
$f_c = R/(2\pi L)$, la componente de rizo de alta frecuencia introducida 
por la conmutación es atenuada de forma natural, siempre que se cumpla 
la condición $f_s \gg f_c$. Bajo esta condición, la tensión efectiva 
sobre el solenoide puede aproximarse con la expresión de la 
ecuación~\eqref{eq:pwm_avg}, lo que permite tratar $D$ como una entrada 
de control continua y lineal para efectos del modelado.
